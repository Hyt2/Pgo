 ...
 <div class="wrapper proinfo" style="margin-top: 20px">
  <div class="proinfo-container clearfix">
                        ...
                        <ul>
                            ...
                            @foreach($array as $kk=>$vv)
                                <li onclick="dian(this)">
                                    <a class="goods-attr" href="javascript:void(0);" name="{{$v->attr_name}}"  value="{{$vv}}">{{$vv}}</a>
                                </li>
                            @endforeach
                        </ul>
                    </dd>
   ...
   <dl style="margin-top: 1px;" class="proinfo-num" id="buycount">
       ...
       <dt >
           库存:(<sanp id="goods_kc" value="{{$goods->goods_kc}}">{{$goods->goods_kc}}</sanp>)件
       </dt>
    ...
    <div class="mainbtns clearfix">
     ...
     <a style="" href="javascript:buyNowTime(this);" class="btn-buynow" name="item_122639559_gmq_ljgm" id="buyNowAddCart"><span>立即购买</span></a>
     <?php 
     ...
     <script>
          function buyNowTime(o)
          {
                ...
                var n = false;
                var attr = $('a.goods-attr').each(function(index, el) {
                ...
                $.ajax({
                    url : '{{ url("buy") }}',
                    data: {'cnt': buyNum, 'goods_id': goodsId, '_token': '{{ csrf_token() }}'},
                    type: 'POST',
                    dataType: 'json',
                    success: function(data) {
                        // 还未登录
                        if (data == 0) {
                            window.location.href = '{{ url("ck") . "?id=" . $goods->id }}';
                        }


                        if (data == 1) {
                            window.location.href = '{{ url("ck") . "?a=orderSuccess" }}';
                        }
                    }
                })
          }
     </script>
     <a href="javascript:addCart(this);" class="btn-addcart" name="item_122639559_gmq_buy01" id="addCart"><span>加入购物车</span></a>
    </div>
        ...
        function addCart(o)
          {
                ...
                var attr = $('li.selected a.goods-attr');

                var data = {};
                attr.each(function(index, el) {
                ...
                var u = '<?php echo !session('UserInfo') ? 0 : 1 ?>';
                if (u == '0') {
